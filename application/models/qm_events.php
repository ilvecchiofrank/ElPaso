...
class QM_Events extends CI_Model {
    ...
    public function searchPreguntasParticipantes($parametros){
        $query = $this->db->query("SELECT a.actividadpersona_pregunta_id as id, b.preguntadescripciontxt as categoriatxt,a.preguntacategoriaid as categoria, REPLACE(a.pregunta_txt,'\n',' ') as inquietud, if(a.respuesta_txt IS NULL, ' ', REPLACE(a.pregunta_txt,'\n',' ')) as respuesta, if(a.pregunta_categorizada_id IS NULL, 0, a.pregunta_categorizada_id) as preguntaCategoria, if(a.respuestacategoriaid IS NULL, 0, a.respuestacategoriaid) as respuestaCategoria FROM actividadpersona_preguntas a
